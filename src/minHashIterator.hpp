...
public:
  ...
  minHashResultIterator<HF> operator*() const {
    return minHashResultIterator<HF>(this);
  }
  ...
  inline int getKmerPosition() const {
    return p;
  }
...
struct minHashResultIterator {
  ...
  const minHashResult& operator*() const {
    return p->v[pos];
  }
  const minHashResult* operator->() const {
    return &(p->v[pos]);
...
public:
  ...
  preAllocMinHashResultIterator<HF> operator*() const {
    return preAllocMinHashResultIterator<HF>(*this);
  }
